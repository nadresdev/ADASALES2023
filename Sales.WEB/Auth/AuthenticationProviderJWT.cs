...
namespace Sales.WEB.Auth
{
    public class AuthenticationProviderJWT : AuthenticationStateProvider, ILoginService
    {
        ...
        private readonly AuthenticationState _anonimous;//es un usuario que no tiene nada
        ...
        {
            var token = await _jSRuntime.GetLocalStorage(_tokenKey); //Mira si el token existe sino, devuelve usuario anonimo
            if (token is null)
            {
                return _anonimous;
            }
        ...
        private AuthenticationState BuildAuthenticationState(string token)
        {
            ...
            return new AuthenticationState(new ClaimsPrincipal(new ClaimsIdentity(claims, "jwt")));//devuelve un usuario con todos los claims
        }
        ...
        public async Task LoginAsync(string token)
        {
            ...
            NotifyAuthenticationStateChanged(Task.FromResult(authState));//notifica que estoy logeado
        }

        public async Task LogoutAsync()
        {
            await _jSRuntime.RemoveLocalStorage(_tokenKey);//remueve el tioken
            _httpClient.DefaultRequestHeaders.Authorization = null;//baja los headers
